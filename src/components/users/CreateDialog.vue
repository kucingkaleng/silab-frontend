<template>
  <v-dialog v-model="dialog" max-width="500px">
    <v-btn small color="primary" slot="activator">Add New <v-icon right>mdi-account-multiple-plus</v-icon></v-btn>
    <v-card>
      <v-card-title>
        <h5 class="text-xs-center headline">New Data</h5>
      </v-card-title>
      ...
      <v-card-text>
        <v-container grid-list-md>
          <v-layout wrap>
            <v-flex xs12>
              <v-text-field v-model="form.nomor_induk" label="Nomor Induk" :rules="nomorIndukRules" required></v-text-field>
            </v-flex>
            <v-flex xs12>
...
export default {
  ...
  methods: {
    refresh (){
      this.$root.$emit('refresh','Data added successfully.')
    },
    validate () {
      if (this.$refs.form.validate()) {
				this.snackbar = true
				if (this.valid) {
					this.axios.post(`${this.patcher}`, {
